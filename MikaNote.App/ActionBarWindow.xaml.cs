...
public partial class ActionBarWindow : Window
{
    ...
    public void ShowAt(double left, double top, double width)
    {
        if (!_hasBeenShown)
        {
            Show();
        ...
        _isPaneVisible = true;
        UpdatePlacement(left, top, width);
    }
    ...
    {
        ActionBarRootBorder.CornerRadius = new CornerRadius(Math.Max(0, cornerRadius));
    }
    ...
    public bool ContainsScreenPoint(System.Windows.Point screenPoint)
    {
        ...
        return screenPoint.X >= Left
            && screenPoint.X < Left + Width
            && screenPoint.Y >= Top
            && screenPoint.Y < Top + Height;
    }
    ...
    public void UpdatePlacement(double left, double top, double width)
    {
        ...
        int deviceWidth = Math.Max(1, (int)Math.Round(Width * transform.M11));
        int deviceHeight = Math.Max(1, (int)Math.Round(Height * transform.M22));

        if (_isDesktopAttached)
        {
            DesktopWindowHost.UpdateBounds(_windowHandle, x, y, deviceWidth, deviceHeight);
        }
    }
    ...
    private void MoveOffscreen()
    {
        ...
        int deviceWidth = Math.Max(1, (int)Math.Round(Width * transform.M11));
        int deviceHeight = Math.Max(1, (int)Math.Round(Height * transform.M22));
        DesktopWindowHost.UpdateBounds(_windowHandle, -32000, -32000, deviceWidth, deviceHeight);
    }
}
